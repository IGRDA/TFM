...
def fourierPricer2(K,S0,payoff,r,T,cf,udep,mcm,xmax=2000,limit=1000):
        ...
        call = S0*np.exp(-mcm*T) - np.sqrt(S0 * K) * np.exp(-(r+mcm)*T/2)/np.pi * \
               quad(integrand, 0, xmax, limit=limit)[0]
        ...
        if payoff=="put":
            return call - S0*np.exp(-mcm*T) + K*np.exp(-r*T) # Call-put parity
def fourierPricer3(K,S0,payoff,r,T,cf,udep,mcm,xmax=2000,limit=1000):
        ...
        call = S0 - np.sqrt(S0 * K) * np.exp(-(r)*T/2)/np.pi * \
               quad(integrand, 0, xmax, limit=limit)[0]
